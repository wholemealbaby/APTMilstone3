...
// Searches for a tile with the given letter 
// in the list and returns its pointer
Tile* TileList::searchPos(int posX, int posY){
   bool found = false;
   Tile* currentTile = head;
   while (found != true && currentTile != nullptr){
...
// Inserts a Tile at the given index
void TileList::insert(Tile* incomingTile, int index){
   if (length == 0){
      head = new Tile(*incomingTile);
      tail = head;
   }

   // First element case
   else if (index == 0) {
      head->prev = new Tile(*incomingTile, nullptr, head);
      head = head->prev;
   }

   // Last element case
   else if (index == length || index == -1){
      tail->next = new Tile(*incomingTile, tail, nullptr);
      tail = tail->next;
   }
   ...
   // Average case
      ...
      prexisitngTile->prev->next = new Tile 
      (*incomingTile, prexisitngTile->prev, prexisitngTile);
